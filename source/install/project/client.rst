...
The Client Control Panel is for customers only and is where all management is commanded.
...
* Change directory to the *gwhcp* user home.
    ...
    ``git clone https://github.com/gwhcp/client.git``
...
.. note::
    ...
    The path to `manage.py` is located in `client/gwhcp_client/`.
...
* Create uWSGI configuration
    ...
    ``python manage.py uwsgi_create_config gwhcp /home/gwhcp/client/gwhcp_client/``
...
Before accessing the Client Control panel for the first time, all static files must first be compiled.
